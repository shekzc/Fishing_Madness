...
public class BlueFish : MonoBehaviour
{
    ...
    void Init()
    {
        ...
        GameObject j1 = new GameObject("Point1");
        j1.transform.SetParent(waypoints.transform);
        j1.transform.position = root.transform.position;

        GameObject j2 = new GameObject("Points2");
        j2.transform.SetParent(waypoints.transform);
        j2.transform.position = root.transform.position;

        points = new List<Transform>();
        points.Add(j1.transform);
        points.Add(j2.transform);
    ...
    void MoveToNextPoint()
    {
        ...
        if (goalPoints.transform.position.x > transform.position.x)
            transform.localScale = new Vector3((float)0.03908876, (float)0.03772137, (float)0.03772137);
        else
            transform.localScale = new Vector3((float)-0.03908876, (float)0.03772137, (float)0.03772137);
